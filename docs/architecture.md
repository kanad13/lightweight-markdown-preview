...
1. **Heading Extraction:** All markdown headings (h1-h6) are extracted and stored for TOC generation
2. **Extraction:** Mermaid and math blocks are extracted and preserved before markdown parsing (prevents escaping of special syntax)
3. **Rendering:** `marked` converts markdown to HTML
4. **ID Assignment:** Unique IDs are assigned to headings for anchor linking
5. **Restoration:** Preserved blocks are restored with original delimiters intact
6. **Path Resolution:** Image `src` attributes are converted to webview-accessible URIs, handling:
...
The TOC sidebar provides quick navigation for documents with multiple sections.

### How It Works

1. **Heading Detection:** During markdown parsing, all headings are extracted with their level (h1-h6) and text content
2. **Nested Structure:** Headings are organized into a nested list respecting their hierarchy (h1 > h2 > h3, etc.)
3. **Anchor Links:** Each heading is assigned a unique URL-friendly ID for smooth anchor linking
4. **Active Section Tracking:** An IntersectionObserver tracks which section is in the viewport and highlights the corresponding TOC link
5. **Auto-Scroll:** The TOC sidebar automatically scrolls to keep the active link visible

### Visual Feedback

- **Fixed Sidebar:** 280px fixed-width panel on the left side of the preview
- **Hierarchy Styling:** Different font sizes for different heading levels (h1 larger, h2-h3 medium, h4-h6 smaller)
- **Active Indicator:** Current section highlighted in blue with white text
- **Hover State:** Links show hover effects for better interactivity

### User Interaction

- **Click to Navigate:** Clicking any TOC link smoothly scrolls to that section
- **Scroll Tracking:** As the user scrolls the document, the TOC automatically updates to show the current section
- **Mobile Responsive:** On screens < 768px, the sidebar moves to the top
